...
// Para cada key que esté en ColumnKey (o sea, 'pendientes' y 'hechas'), su valor será un Task[] (array de tareas).
type Columns = {
  [key in ColumnKey]: Task[]; //“Voy a tener un objeto (columns) que debe tener exactamente dos claves: 'pendientes'
  //  y 'hechas', y cada una tendrá un array de tareas (Task[]) como valor.”
};

const initialTasks: Task[] = [
  //Gracias a esa anotación : Task[], TypeScript va a verificar que los
  // objetos dentro del array cumplan con esa estructura exacta.
...
export default function App() {
  const [columns, setColumns] = useState<Columns>({
    // Voy a usar useState para manejar un estado que debe seguir
    // la estructura del tipo Columns
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const sourceColumn = (Object.keys(columns) as ColumnKey[]).find(
      (
        col // .find((col) =>  Busca la primera columna donde se encuentre el ítem
      ) =>
    ...
    if (
      ...
      setColumns((prev) => ({
        // prev es el estado anterior de las columnas
        ...prev, // Copia todas las columnas existentes sin modificarlas.
        [sourceColumn]: prev[sourceColumn].filter(
          // Elimina el ítem arrastrado de su columna original.
